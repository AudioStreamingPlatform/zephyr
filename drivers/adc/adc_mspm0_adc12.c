...
#define MSPM0_ADC_CLOCK_DIV(div) DT_CAT(DL_ADC12_CLOCK_DIVIDE_, div)
...
#define MSPM0_ADC_INIT(index)                                                                 \
	...
	static const struct adc_mspm0_cfg adc_mspm0_cfg_##index = {                           \
		...
		.clock_cfg = {                                                                \
			.clockSel = DT_INST_PROP(index, ti_clk_source),                       \
			.freqRange = DT_INST_PROP(index, ti_clk_range),                       \
			.divideRatio = MSPM0_ADC_CLOCK_DIV(DT_PROP(DT_DRV_INST(index),        \
							   ti_clk_div)),                      \
		}                                                                             \
	};                                                                                    \
	                                                                                      \
