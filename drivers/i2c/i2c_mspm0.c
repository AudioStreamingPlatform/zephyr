...
#endif /* CONFIG_I2C_MSPM0_THREAD_SUPPORT */
...
static void i2c_mspm0_target_start_watchdog(struct i2c_mspm0_data *data)
{
	const struct i2c_mspm0_config *config = data->cfg;
...
static bool i2c_mspm0_has_thread_support() {
#ifdef CONFIG_I2C_MSPM0_THREAD_SUPPORT
	return k_current_get() == &i2c_mspm0_target_thread;
#else
	return false;
#endif
}
					  ...
					  struct i2c_target_config *target_config)
{
	 ...
	 *  */
	const bool omit_lock = i2c_mspm0_has_thread_support();
...
static int i2c_mspm0_init(const struct device *dev)
{
	...
	if (config->watchdog_timer) {
		...
		data->watchdog_timer_cfg.callback = i2c_mspm0_panic;
...
#define MSPM0_I2C_CLK_DIV(div)			DT_CAT(DL_I2C_CLOCK_DIVIDE_, div)
...
#define MSP_I2C_INIT_FN(index)                                                                     \
	...
	static const struct i2c_mspm0_config i2c_mspm0_cfg_##index = {                             \
		...
		.clock_cfg = {                                                                     \
			...
			.divideRatio = MSPM0_I2C_CLK_DIV(DT_PROP(DT_DRV_INST(index),               \
							 ti_clk_div)),                             \
		},                                                                                 \
