...
static int i2c_mspm0g3xxx_target_register(const struct device *dev, struct i2c_target_config *target_config){
	...
	LOG_DBG("Register %s target on addr=%u", target_type == TARGET_TYPE_PRIMARY ? "primary" : "alternate", target_config->address);
	...
	if(data->is_target == true){
		...
		if (target_type == TARGET_TYPE_PRIMARY) {
			...
			DL_I2C_setTargetOwnAddressAlternate((I2C_Regs *)config->base, target_config->address);
...
static int i2c_mspm0g3xxx_target_unregister(const struct device *dev, struct i2c_target_config *target_config){
	...
	if (data->target_config_primary != NULL && data->target_config_primary->address == target_config->address) {
	...
	} else if (data->target_config_alternate != NULL && data->target_config_alternate->address == target_config->address) {
		...
		LOG_ERR("Failed to unregister device as none was found at addr=%u", target_config->address);
		k_sem_give(&data->i2c_busy_sem);
		return -ENODEV;
	}
...
struct i2c_target_config* i2c_mspm0g3xxx_config_from_addr(const struct i2c_mspm0g3xxx_data* data, uint32_t addr) 
{
	...
	switch (DL_I2C_getPendingInterrupt((I2C_Regs *)config->base)) {
		...
		case DL_I2C_IIDX_TARGET_RXFIFO_TRIGGER:
			if(data->state == I2C_mspm0g3xxx_TARGET_STARTED){
				...
				if(tconfig != NULL && tconfig->callbacks->write_requested != NULL){
					data->target_rx_valid = tconfig->callbacks->write_requested(tconfig);
				}
			}
			...
			if(tconfig != NULL && tconfig->callbacks->write_received != NULL) {
				...
				while (DL_I2C_isTargetRXFIFOEmpty((I2C_Regs *)config->base) != true) {
					if (data->target_rx_valid == 0){
						...
						data->target_rx_valid = tconfig->callbacks->write_received(tconfig, nextByte);
					} else {
		...
		case DL_I2C_IIDX_TARGET_TXFIFO_TRIGGER:
			...
			if(tconfig != NULL && tconfig->callbacks->read_requested != NULL){
				uint8_t nextByte;
				data->target_tx_valid = tconfig->callbacks->read_requested(tconfig, &nextByte);
		...
		case DL_I2C_IIDX_TARGET_TXFIFO_EMPTY:
			if(tconfig != NULL && tconfig->callbacks->read_processed != NULL){
				...
				uint8_t nextByte;
				if(data->target_tx_valid == 0) {
					data->target_tx_valid = tconfig->callbacks->read_processed(tconfig, &nextByte);
				}
